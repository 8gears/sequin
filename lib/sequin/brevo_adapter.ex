defmodule Sequin.Swoosh.Adapters.Brevo do
  @moduledoc ~s"""
  An adapter that sends email using the Brevo API.

  For reference: [Brevo API docs](https://developers.brevo.com/)
  ...
  **This adapter requires an API Client.** Swoosh comes with Hackney, Finch and Req out of the box.
  ...
  def deliver(%Email{} = email, config \\ []) do
    if !config[:api_key] and Application.get_env(:sequin, :self_hosted) do
      ...
      case Brevo.send_transactional(request, api_key: config[:api_key]) do
  ...
  defp prepare_request(email) do
    %TransactionalRequest{
      ...
      template_id: email.provider_options[:template_id],
      params: email.provider_options[:params] || %{}
    }
  end
end
